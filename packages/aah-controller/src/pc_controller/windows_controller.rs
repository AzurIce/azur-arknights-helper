...
use windows::Win32::{Foundation::{BOOL, HWND, LPARAM, RECT}, Graphics::Gdi::{BitBlt, CreateCompatibleBitmap, CreateCompatibleDC, DeleteDC, DeleteObject, GetDC, GetDIBits, ReleaseDC, SelectObject, BITMAPINFO, BITMAPINFOHEADER, BI_RGB, DIB_RGB_COLORS, SRCCOPY}, UI::{Input::KeyboardAndMouse::{GetKeyboardLayout, VIRTUAL_KEY}, WindowsAndMessaging::{
        EnumWindows, GetDesktopWindow, GetForegroundWindow, GetSystemMetrics, GetWindowRect, GetWindowTextW, GetWindowThreadProcessId, IsWindowVisible, SetForegroundWindow, SM_CXSCREEN, SM_CYSCREEN
    }}};
use enigo::{
...
impl WindowsController {
    ...
    fn impl_swipe(&self, from_x: i32, from_y: i32, to_x: i32, to_y: i32, duration_ms: u64) -> Result<(), MyError> {
        ...
        // 逐步移动
        for i in 1..=steps {
            enigo.move_mouse((x_step * i as f64) as i32, (y_step * i as f64) as i32, Rel).unwrap();
    ...
    fn impl_move_mouse_relative(&self, dx: i32, dy: i32) -> Result<(), MyError> {
        let mut enigo = Enigo::new(&Settings::default())?;
    ...
    fn impl_move_mouse_absolute(&self, x: i32, y: i32) -> Result<(), MyError> {
        let mut enigo = Enigo::new(&Settings::default())?;
    ...
    fn impl_location(&self) -> Result<(i32, i32), MyError> {
...
fn convert_key_to_win32vk(key: Key) -> Result<VIRTUAL_KEY, MyError> { 
    use windows::Win32::UI::Input::KeyboardAndMouse::{
        ...
        VK_PACKET, VK_PAUSE, VK_PLAY, VK_PRIOR, VK_PROCESSKEY, VK_Q, VK_R,
    ...
    let vk = match key {
        ...
        Key::Unicode(c) => 'unicode_handling: {
            ...
            match c {
                ...
                '\0' => {
                    return Err(MyError::S("Invalid mapping".to_string()));
                }
                _ => (),
            }

            // let layout = Enigo::get_keyboard_layout();
            let current_window_thread_id = unsafe { GetWindowThreadProcessId(GetForegroundWindow(), None) };
            ...
            if utf16_surrogates.len() != 1 {
                return Err(MyError::S("Character can't be mapped to only one virtual key".to_string()));
            }
            ...
            if vk < 0 {
                return Err(MyError::S("Character can't be mapped to virtual key".to_string()));
            }
            VIRTUAL_KEY(vk as u16)
        }
        Key::Other(v) => {
            let Ok(v) = u16::try_from(v) else {
                return Err(MyError::S("virtual keycodes on Windows have to fit into u16".to_string()));
            };
            VIRTUAL_KEY(v)
        }
        Key::Meta | Key::LWin => VK_LWIN,
        _ => return Err(MyError::S("Invalid mapping".to_string())),
    };
