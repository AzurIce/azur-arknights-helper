...
impl Runnable<AahCore> for Copilot {
    type Output = ();
    fn execute(&self, executor: &AahCore) -> anyhow::Result<Self::Output> {
        ...
        let level = get_level(
            ...
            executor.resource.root.join("levels.json"),
        )
        .unwrap();
        ...
        match start_pre.execute(executor) {
            Ok(_) => {
        ...
        loop {
            let output = battle_analyzer.execute(executor)?;
            if output.battle_state != BattleState::Unknown {
                break;
            }
            thread::sleep(Duration::from_secs_f32(0.5));
        }
        ...
        let skill_ready_template =
            get_template("battle_skill-ready.png", &executor.resource.root)?.to_luma32f();
             ...
             deployed_operators: &HashMap<String, (u32, u32)>| {
                ...
                for oper in auto_skill_operators.iter() {
                    if let Some(position) = deployed_operators.get(oper).cloned() {
                        if let Ok(screen) = executor.screen_cache_or_cap() {
                            ...
                            if skill_ready {
                                ...
                                // 32 187 64x64
                                if UseSkill::new(&level, &position).execute(executor).is_ok() {
                                    // aah.emit_task_evt(TaskEvt::Log(format!(
                                    //     "[INFO]: {oper} 技能已使用"
                                    // )));
        ...
        loop {
            let output = battle_analyzer.execute(executor)?;
            if output.battle_state == BattleState::Completed {
                break;
            }
            // Execute step
            if let Some((idx, step)) = cur {
                ...
                battle_analyzer_output = battle_analyzer.execute(executor)?;
                // auto skilling
                ...
                let success = match &step.action {
                    ...
                    CopilotAction::Retreat { operator, .. } => {
                        ...
                        let success = Retreat::new(&level, &position).execute(executor).is_ok();
                        if success {
