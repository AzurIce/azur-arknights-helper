...
pub struct DeployAnalyzer {
    ...
    avatar_images: Vec<ImageBuffer<Luma<f32>, Vec<f32>>>,
...
impl DeployAnalyzer {
    pub fn new<S: AsRef<str>>(res_dir: impl AsRef<Path>, opers: &[S]) -> Self {
        ...
        let images = opers_avatars
            .into_iter()
            .map(|(_, img)| img.to_luma32f())
            .collect::<Vec<ImageBuffer<Luma<f32>, Vec<f32>>>>();
    ...
    pub fn analyze_image(&self, image: &DynamicImage) -> anyhow::Result<DeployAnalyzerOutput> {
        ...
        let res = output.res;
        cprintln!("{log_tag}found {} deploy cards...", res.result.len());
        let mut deploy_cards: Vec<DeployCard> = Vec::with_capacity(res.result.len());
        for Match { rect, .. } in res.result.iter() {
            ...
            let avatar_template = cropped.to_luma32f();
            assert!(avatar_template.width() * avatar_template.height() > 0); // make sure the template is not empty
            let BestMatcherResult { result, .. } = BestMatcher::match_template(
                &self.avatar_images,
                &avatar_template,
